      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-4 mb-4">
        <div>
          ...
          <div className="flex items-center space-x-1.5 text-[10px] text-slate-500 font-bold mt-1">
            ...
            <span>•</span>
            <span className="text-[#b33c56]">
              {activeReport === 'stages'
                ? '(Report for the Month - July 2026)'
                : '(Report for the Month - July 2026)'}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 animate-fade-in pt-4">
          ...
          {activeReport === 'stages' ? (
            <div className="lg:col-span-3 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3 flex flex-col justify-between">
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Response Clearance Rate by Ministry</h3>
                  ...
                  return (
                    <div key={row.id} className="space-y-1 font-semibold">
                      <div className="flex justify-between text-xs text-slate-600">
                        <span className="truncate max-w-[220px]" title={row.wing}>{row.wing}</span>
            ...
            <div className="lg:col-span-3 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3 flex flex-col justify-between">
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Cabinet Notes Pendency Age Distribution</h3>
              <div className="space-y-3.5 pt-2 max-h-[240px] overflow-y-auto pr-1">
                {dynamicReportRows.filter(r => r.total > 0).map((row) => {
                  const totalAgeNotes = row.age0_50 + row.age51_80 + row.ageOver80;
                  const percentOver80 = Math.round((row.ageOver80 / (totalAgeNotes || 1)) * 100);
                  const gradientClass = 'bg-gradient-to-r from-rose-500 to-red-600';
                  return (
                    <div key={row.id} className="space-y-1 font-semibold">
                      <div className="flex justify-between text-xs text-slate-600">
                        <span className="truncate max-w-[220px]" title={row.wing}>{row.wing}</span>
            ...
            <div className="lg:col-span-2 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
              ...
              <div className="space-y-3.5 pt-2">
                {[...dynamicReportRows]
                  ...
                  .slice(0, 5)
                  .map((row, idx) => {
                    const percent = totals.total > 0 ? (row.total / totals.total) * 100 : 0;
                    const colors = [
                      'bg-indigo-500',
                      'bg-blue-500',
                      'bg-sky-500',
                    ...
                    const color = colors[idx] || 'bg-slate-500';
                    return (
                      <div key={row.id} className="space-y-1 font-semibold text-xs text-slate-700">
                        <div className="flex justify-between">
                          <span className="truncate max-w-[150px]" title={row.wing}>{row.wing}</span>
              ...
              <div className="text-[10px] text-center text-slate-400 font-semibold border-t border-slate-200/60 pt-2.5 mt-2">
                Top 5 ministries by incoming Cabinet Notes volume
              </div>
            </div>
          ) : (
            <div className="lg:col-span-2 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">

...
import { ChevronDown, Globe, Type, Sun, Moon, Bell } from 'lucide-react';
...
export default function Header({ onLogout, onProfileClick, onUserManagementClick }) {
    ...
    axios
      ...
      .then((res) => {
        ...
        const matched = users.find(
          (u) => u.email && u.email.toLowerCase() === decoded.email.toLowerCase()
        );
        if (matched) {
  ...
  useEffect(() => {
    ...
    const loadGlobalNotifCount = async () => {
      try {
        ...
        const clearedDropCount = Number(localStorage.getItem('sagar_cleared_drop_count') || 0);
        const readDropCount = Number(localStorage.getItem('sagar_read_drop_count') || 0);

        let count = 0;
        // Show badge count 1 when there are pending requests that have not been cleared or read
        if (pending.length > 0 && pending.length > clearedDropCount && pending.length > readDropCount) {
          count = 1;
        }

        if (isMounted) setDropCount(count);
      } catch {
        // silent fallback
      }
    };
    ...
    const interval = setInterval(loadGlobalNotifCount, 15000);
    window.addEventListener('drop-request-updated', loadGlobalNotifCount);
    window.addEventListener('notifications-updated', loadGlobalNotifCount);
    return () => {
      isMounted = false;
      clearInterval(interval);
      window.removeEventListener('drop-request-updated', loadGlobalNotifCount);
      window.removeEventListener('notifications-updated', loadGlobalNotifCount);
    };
  }, []);
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (
        ...
        langDetailsRef.current &&
        !langDetailsRef.current.contains(event.target) &&
        avatarDetailsRef.current &&
        !avatarDetailsRef.current.contains(event.target)
      ) {
        setShowFontSlider(false);
        setShowAvatarDropdown(false);
        if (langDetailsRef.current) langDetailsRef.current.open = false;
    ...
    <header className="bg-gradient-to-r from-[#0a2540] via-[#0f417a] to-[#008ca3] text-white shadow-lg border-b border-[#008ca3]/30">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Left: Emblem, Government branding & Sagarmanthan Logo (GIGW Layout) */}
          ...
          <div className="flex items-center space-x-3 sm:space-x-5">
            ...
            <div className="hidden lg:flex flex-col text-right">
              ...
              <span className="text-xs font-medium text-white flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-emerald-400 inline-block border-black" />
                {getGreeting()}, <strong className="text-white">{userName || 'User'}</strong> |{' '}
                <span className="text-slate-300 font-normal">{userOrg || '—'}</span>
              </span>
            </div>
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 border-l border-white/20 pl-2 sm:pl-4">
              ...
              <button
                ...
                className="relative p-1.5 rounded-lg transition-all cursor-pointer text-slate-300 hover:text-white hover:bg-white/10 flex items-center justify-center"
                title="Notifications & Alerts"
                aria-label="Open Notifications"
              >
                <Bell className="h-4 w-4" />
                {dropCount > 0 && (
                  <span className="absolute -top-1 -right-1.5 min-w-[16px] h-4 px-1 flex items-center justify-center text-[10px] font-extrabold text-white bg-rose-500 rounded-full border border-slate-900 shadow-sm leading-none pointer-events-none">
              ...
              <details
                ...
                onToggle={(e) => {
                  if (e.target.open) handleDropdownOpen('lang');
                }}
              >
                {/* Dropdown Trigger Button */}
                <summary
                  className="list-none outline-none cursor-pointer p-1.5 text-slate-300 hover:text-white hover:bg-white/10 rounded-lg transition-colors flex items-center space-x-1"
              ...
              <div className="relative" ref={sliderRef}>
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-all cursor-pointer ${
                    showFontSlider
                      ? 'text-cyan-300 bg-white/15 shadow-inner'
                ...
                {showAvatarDropdown && (
                  <div className="absolute right-0 w-48 mt-2 py-1 bg-[#0a2540] border border-white/10 rounded-xl shadow-2xl z-50 transition-all duration-200">
                    <a
                      ...
                      className="block px-4 py-2 text-xs text-slate-200 hover:bg-white/10 hover:text-white"
                    >
                      Profile Settings
                    </a>
                    {isOrgSeniorOfficer() && (
                    ...
                    <a
                      ...
                      className="block px-4 py-2 text-xs text-red-400 hover:bg-white/10 font-medium"
                    >
                      Sign Out
                    </a>
                  </div>

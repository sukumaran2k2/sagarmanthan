...
import { fetchNewCourseUpgradationReport } from '../../api';
...
// Metric name confirmed exactly from the backend's SQL pivot: the actual
// stored value is "Number of New Couses" (typo, not "Courses") -- matched
// verbatim here since it's a real API response key, not just display text.
export default function NewCourseUpgradationReports() {
  ...
  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchNewCourseUpgradationReport()
      .then((res) => {
        ...
        const offeredRow = data.find((r) => r.Metric === 'Number of Courses Offered') || {};
        const upgradedRow = data.find((r) => r.Metric === 'Number of New Couses') || {};

        setChartData(yearCols.map((yr) => ({
          year: yr,
          offered: Number(offeredRow[yr] || 0),
      ...
      .catch((err) => {
        console.error('Error loading New Course Upgradation report:', err);
  ...
  useLayoutEffect(() => {
    ...
    const yAxis = chart.yAxes.push(am5xy.ValueAxis.new(root, { renderer: yRenderer, min: 0, extraMax: 0.3 }));

    const upgradedSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Number of New Courses', xAxis, yAxis,
        valueYField: 'upgraded', categoryXField: 'year',
        tooltip: am5.Tooltip.new(root, { labelText: 'Number of New Courses: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    upgradedSeries.columns.template.setAll({ fill: am5.color(0x20b2aa), stroke: am5.color(0x20b2aa), cornerRadiusTL: 3, cornerRadiusTR: 3, width: am5.percent(50) });
    upgradedSeries.bullets.push((root) => {
    ...
    upgradedSeries.data.setAll(chartData);

    const offeredSeries = chart.series.push(
      am5xy.LineSeries.new(root, {
        name: 'Number of Courses Offered', xAxis, yAxis,
        valueYField: 'offered', categoryXField: 'year',
        stroke: am5.color(0xff5733), fill: am5.color(0xff5733),
        tooltip: am5.Tooltip.new(root, { labelText: 'Number of Courses Offered: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    offeredSeries.strokes.template.setAll({ strokeWidth: 2, stroke: am5.color(0xff5733) });
    offeredSeries.bullets.push(() => am5.Bullet.new(root, {
      sprite: am5.Circle.new(root, { radius: 4, fill: am5.color(0xff5733), stroke: am5.color(0xffffff), strokeWidth: 2 })
    }));
    offeredSeries.bullets.push((root) => {
      const label = am5.Label.new(root, { fill: am5.color(0xff5733), centerX: am5.p50, centerY: am5.p0, dy: -28, fontSize: 10, fontWeight: '700', populateText: true, text: '{valueY}' });
      return am5.Bullet.new(root, { sprite: label });
    });
    offeredSeries.data.setAll(chartData);
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'imu_new_course_upgradation_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .imu-newcourse-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .imu-newcourse-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .imu-newcourse-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .imu-newcourse-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .imu-newcourse-report-grid .ag-header-cell-label { justify-content: center !important; }
        .imu-newcourse-report-grid .ag-row-even { background: #ffffff !important; }
        .imu-newcourse-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .imu-newcourse-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .imu-newcourse-report-grid .ag-header { background: #0f172a !important; }
        .dark .imu-newcourse-report-grid .ag-row-even { background: #090d16 !important; }

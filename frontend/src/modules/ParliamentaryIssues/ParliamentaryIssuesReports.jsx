...
function ActionStagesPieChart({ totals }) {
  useEffect(() => {
    let root = am5.Root.new("stages-pie-chart");
    ...
    series.data.setAll([
      { category: "Comments Sought", value: totals.commentsSought || 0 },
      { category: "Comments Received", value: totals.commentsReceived || 0 },
      { category: "Extension Sought", value: totals.extensionSought || 0 },
      { category: "Implementation Report", value: totals.implementationReport || 0 },
      { category: "Matter Disposed", value: totals.matterDisposed || 0 }
    ]);
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between h-[340px]">
      <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">Stages Distribution</h3>
      <div id="stages-pie-chart" className="w-full h-[260px]"></div>
    </div>
  );
}

export default function ParliamentaryIssuesReports({ issues = [], mode = 'report' }) {
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 pt-2">
          {/* Chart 1: Matters Disposed by Wing */}
          ...
          {/* Chart 2: Ministry Action Stages */}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Analytical Report Header Banner */}
      ...
      {/* Visual Diagrams underneath analytical report */}

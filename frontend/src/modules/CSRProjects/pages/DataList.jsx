...
export default function DataList({
  ...
  const columnDefs = useMemo(() => [
    ...
    },
    {
      ...
      valueFormatter: (p) => (p.value != null && p.value !== '' ? Number(p.value).toLocaleString() : '-'),
      cellRenderer: (p) => (
    ...
    },
    {
      ...
      cellRenderer: (p) => (
        <div className="w-full flex items-center justify-center text-center">
          <span className="font-semibold text-slate-700 dark:text-slate-200">{p.value || 'Project yet to start'}</span>
        </div>
      )
    },
    {
      ...
      cellRenderer: (p) => {
        ...
        return (
          <div className="flex items-center justify-center space-x-1.5 h-full w-full">
            <button
              type="button"
              onClick={() => handleOpenDetailModal(p.data)}
            ...
            <button
              type="button"
              onClick={() => handleOpenGalleryModal(p.data)}
    ...
    <div className="space-y-4 animate-fade-in text-slate-800 dark:text-slate-100">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
        ...
        <div className="flex flex-col lg:flex-row gap-3 items-center justify-between border-b border-slate-100 dark:border-slate-800/60 pb-4">
          ...
          {/* Right-aligned Tools: Search bar, Rows, Total, Visibility, Copy, Export, Add New */}
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto justify-end">
            ...
            <div className="relative min-w-[200px] flex-1 sm:flex-initial">
              ...
              <input
                type="text"
                placeholder="Search project, organisation, remarks..."
                value={searchTerm}
              ...
              {colDropdownOpen && (
                <div className="absolute right-0 mt-1.5 w-56 max-h-80 overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-lg p-2 z-50 animate-fade-in flex flex-col space-y-0.5 dark:bg-slate-900 dark:border-slate-800">
                  <div className="flex items-center justify-between px-2 py-1 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <button
                      type="button"
                      onClick={() => setVisibleCols({ sno: true, org: true, financial_year: true, project_name: true, project_value: true, project_status: true, completed_on: true, remarks: true, actions: true })}
            ...
            <CopyButton data={filteredProjects} columns={exportColumns} color="#0f417a" triggerNotification={triggerNotification} />
            <ExportDropdown data={filteredProjects} columns={exportColumns} fileName="CSR_Projects_List" title="CSR Projects List" color="#0f417a" hoverColor="#1e5ea8" triggerNotification={triggerNotification} />
          </div>
        </div>
        ...
        {showFilterPanel && (
          <div className="bg-slate-50 dark:bg-slate-950/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1">CSR Focus Area</label>
                <select
                  value={selectedFocus}
                  onChange={(e) => { setSelectedFocus(e.target.value); setCurrentPage(1); }}
                  className="w-full px-3 py-2 text-xs font-semibold bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-1 focus:ring-blue-500 text-slate-800 dark:text-slate-200 cursor-pointer truncate"
                >
                  <option value="">All Focus Areas</option>
                  {CSR_FOCUS_AREAS.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}
                </select>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1">Project Status</label>
                <select
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border-l-4 border-l-[#0f417a]">
            <div className="sticky top-0 bg-gradient-to-r from-[#0f417a] to-[#1e5ea8] px-6 py-4 flex items-center justify-between text-white z-10">
              <div>
                <h3 className="text-sm font-black uppercase tracking-wider font-display">CSR Project Overview</h3>
              ...
              <button onClick={() => setSelectedProjectForView(null)} className="text-white/80 hover:text-white p-1 rounded-lg hover:bg-white/10 transition cursor-pointer">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 border-t border-slate-200 dark:border-slate-800 text-xs">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-[#0f417a] dark:text-blue-400">{selectedProjectForView.project_status}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Physical %</span>
                    <span className="font-black text-emerald-600">{selectedProjectForView.physical_progress != null ? `${selectedProjectForView.physical_progress}%` : '-'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Financial %</span>
                    <span className="font-black text-blue-600">{selectedProjectForView.financial_progress != null ? `${selectedProjectForView.financial_progress}%` : '-'}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs">
                  <div>
                    ...
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{selectedProjectForView.project_received_from || '-'}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block">Target Beneficiaries</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-300">{selectedProjectForView.target_beneficiaries || '-'}</span>
                  </div>
                </div>
                ...
                {selectedProjectForView.impact_possible_outcome && (
                  <div className="pt-2 text-xs">
                    ...
                    <p className="text-slate-700 dark:text-slate-300 font-medium">{selectedProjectForView.impact_possible_outcome}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                <h5 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Yearly Expenditure Breakdown</h5>
                {loadingExpenditures ? (
                  <div className="p-4 text-center text-xs text-slate-400">Loading expenditures...</div>
                ) : projectExpenditures.length === 0 ? (
                  <div className="p-3 bg-slate-50 dark:bg-slate-950 rounded-lg text-xs text-slate-400 text-center">No yearly expenditure records logged.</div>
                ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 bg-gradient-to-r from-purple-700 to-indigo-700 px-6 py-4 flex items-center justify-between text-white z-10">
              <div>
                <h3 className="text-sm font-black uppercase tracking-wider font-display">Project Photo Gallery</h3>
                <p className="text-[10px] text-purple-200 font-semibold mt-0.5">{selectedProjectForGallery.project_name}</p>
              </div>
              <button onClick={() => setSelectedProjectForGallery(null)} className="text-white/80 hover:text-white p-1 rounded-lg hover:bg-white/10 transition cursor-pointer">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="p-6">
              {loadingGallery ? (
                <div className="py-12 text-center text-slate-400 font-semibold">Loading gallery photos...</div>
              ) : galleryImages.length === 0 ? (
                <div className="py-12 text-center text-slate-400 font-semibold">No photos uploaded for this project.</div>
              ) : (

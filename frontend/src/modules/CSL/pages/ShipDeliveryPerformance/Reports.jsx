...
import { fetchShipDeliveryPerformanceReport } from '../../api';
...
export default function ShipDeliveryPerformanceReports() {
  ...
  const title = 'Form No. K-4.3 - Abstract - CSL Ship Delivery Performance';

  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchShipDeliveryPerformanceReport()
      .then((res) => {
        ...
        const transformed = data.map((item) => {
          ...
          return {
            financialQuarter: `${item['Quarter']} FY ${startYear}-${shortYear}`,
            shipOrders: Number(item['Ship Orders Received'] || 0),
            shipsDelivered: Number(item['Number of Ship Delivered on Time'] || 0),
            sortKey: `${item['Financial Year']}-${item['Quarter']}`,
          };
        });
      ...
      .catch((err) => {
        console.error('Error loading Ship Delivery Performance report:', err);
  ...
  useLayoutEffect(() => {
    ...
    // ORDERS RECEIVED -- bars, left axis (matches the legacy site exactly)
    const ordersSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Ship Orders Received', xAxis, yAxis: yAxisLeft,
        valueYField: 'shipOrders', categoryXField: 'financialQuarter',
        tooltip: am5.Tooltip.new(root, { labelText: 'Ship Orders Received: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    ordersSeries.columns.template.setAll({ fill: am5.color(0x87ceeb), stroke: am5.color(0x87ceeb), cornerRadiusTL: 3, cornerRadiusTR: 3, width: am5.percent(60) });
    ordersSeries.columns.template.states.create('hover', { fill: am5.color(0x5ba3d0) });
    ordersSeries.bullets.push((root) => {
    ...
    ordersSeries.data.setAll(chartData);

    // DELIVERED ON TIME -- line, right axis
    const deliveredSeries = chart.series.push(
      am5xy.LineSeries.new(root, {
        name: 'Number of Ship Delivered on Time', xAxis, yAxis: yAxisRight,
        valueYField: 'shipsDelivered', categoryXField: 'financialQuarter',
        stroke: am5.color(0xe85d04), fill: am5.color(0xe85d04),
        tooltip: am5.Tooltip.new(root, { labelText: 'Number of Ship Delivered on Time: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    deliveredSeries.strokes.template.setAll({ strokeWidth: 2, stroke: am5.color(0xe85d04) });
    deliveredSeries.bullets.push(() => am5.Bullet.new(root, {
      sprite: am5.Circle.new(root, { radius: 4, fill: am5.color(0xe85d04), stroke: am5.color(0xffffff), strokeWidth: 2 })
    }));
    deliveredSeries.bullets.push((root) => {
    ...
    deliveredSeries.data.setAll(chartData);
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'csl_ship_delivery_performance_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
        ...
        fileName: 'csl_ship_delivery_performance_report',
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        <div className="flex-1 min-w-[300px]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10.5px] font-black text-[#8c4242] dark:text-blue-400 uppercase tracking-widest">CSL - Ship Delivery Performance Report</span>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="csl_ship_delivery_performance_chart" color="#4b2424" />
      ...
      {error ? (
        ...
        <div className="ag-theme-quartz csl-shipdelivery-report-grid mt-4" style={{ width: '100%' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .csl-shipdelivery-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .csl-shipdelivery-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .csl-shipdelivery-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .csl-shipdelivery-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .csl-shipdelivery-report-grid .ag-header-cell-label { justify-content: center !important; }
        .csl-shipdelivery-report-grid .ag-row-even { background: #ffffff !important; }
        .csl-shipdelivery-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .csl-shipdelivery-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .csl-shipdelivery-report-grid .ag-header { background: #0f172a !important; }
        .dark .csl-shipdelivery-report-grid .ag-row-even { background: #090d16 !important; }
        .dark .csl-shipdelivery-report-grid .ag-row-odd { background: #0f172a !important; }
        .dark .csl-shipdelivery-report-grid .ag-cell { border-right: 1px solid #1e293b !important; color: #e2e8f0 !important; }
      `}} />
      </div>
    </div>
  );

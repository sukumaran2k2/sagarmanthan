...
import { Search, Info, Filter, ChevronDown, X } from 'lucide-react';
...
export default function CapexDataListView({
  ...
  const toggleableCols = useMemo(
    () =>
      (colDefs || [])
        .filter((col) => col.headerName && col.headerName !== 'S.No')
        .map((col) => ({
          key: getColKey(col),
  ...
  const displayColDefs = useMemo(
    () =>
      (colDefs || []).map((col) => {
        ...
        if (!key || col.headerName === 'S.No') return col;
        return {
          ...col,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {viewMode !== 'org' ? (
              ...
              <div>
                ...
                <div className="w-full text-xs px-3 py-2 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-700 dark:text-slate-300">
                  {selectedOrgLabel}
                </div>
              </div>
            )}

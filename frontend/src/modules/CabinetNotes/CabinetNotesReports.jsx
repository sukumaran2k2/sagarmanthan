...
function ActionStagesPieChart({ totals }) {
  useEffect(() => {
    let root = am5.Root.new("stages-pie-chart-cab");
    ...
    series.data.setAll([
      { category: "Draft Prep", value: totals.prep || 0 },
      { category: "Approved Min", value: totals.appMin || 0 },
      { category: "Circulated IMC", value: totals.circIMC || 0 },
      { category: "IMC Rec", value: totals.imcRec || 0 },
      { category: "Prep Final", value: totals.prepFinal || 0 },
      { category: "App Final", value: totals.appFinal || 0 },
      { category: "Adv PMO", value: totals.advPMO || 0 },
      { category: "Approved Cabinet", value: totals.appCab || 0 },
      { category: "Hold", value: totals.hold || 0 },
      { category: "Completed", value: totals.comp || 0 }
    ]);
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between h-[340px]">
      <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">Cabinet Notes Stages Distribution</h3>
      <div id="stages-pie-chart-cab" className="w-full h-[260px]"></div>
    </div>
  );
}
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 pt-2">
          {/* Chart 1: DCN Clearance Progress by Wing */}
          <div className="lg:col-span-3 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">DCN Clearance Progress by Wing</h3>
            <div className="space-y-3.5 pt-2">
              {dynamicReportRows.filter(r => r.total > 0).map((row, idx) => {
                const percent = (row.comp / (row.total || 1)) * 100;
                const gradientClass = 
                ...
                return (
                  <div key={row.id} className="space-y-1 font-semibold">
                    <div className="flex justify-between text-xs text-slate-600">
                      <span>{row.wing}</span>
                      <span>{row.comp} Completed of {row.total} ({Math.round(percent)}%)</span>
                    </div>
          ...
          {/* Chart 2: Notes Status distribution */}
          <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
            <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">DCN Clearance Status</h3>

...
import { createTimeVoyageOffshore, updateTimeVoyageOffshore } from '../../api';
...
export default function TimeVoyageOffshoreInputForm({
  editData = null,
  onBack,
  onSuccess,
  triggerNotification,
  ...
  const [offshoreFleet, setOffshoreFleet] = useState(editData?.total_no_of_offshore_in_fleet ?? '');
  const [timeCharter, setTimeCharter] = useState(editData?.total_no_days_on_offshore_time_charter ?? '');
  const [voyageCharter, setVoyageCharter] = useState(editData?.total_no_days_on_offshore_voyage_charter ?? '');
  const [revenue, setRevenue] = useState(editData?.total_revenue_offshore ?? '');
  const [average, setAverage] = useState(editData?.average_earnings_offshore ?? '');
  ...
  const FIELDS = [
    ...
    ['offshoreFleet', offshoreFleet],
    ['timeCharter', timeCharter],
    ['voyageCharter', voyageCharter],
    ['revenue', revenue],
    ['average', average],
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    const numericValues = [offshoreFleet, timeCharter, voyageCharter, revenue, average].map(Number);
    ...
    try {
      if (isEdit) {
        await updateTimeVoyageOffshore({
          updatefinancialYear: financialYear,
          updateoffshoreFleet: numericValues[0],
          updatetimeCharter: numericValues[1],
          updatevoyageCharter: numericValues[2],
          updateRevenue: numericValues[3],
          updateAverage: numericValues[4],
          ScitimeVoyageoffshoreIdOrg: editData.sci_time_voyage_offshore_id,
          userID: getCurrentUserId(),
        });
        triggerNotification && triggerNotification('Time & Voyage Chartered (Offshore) entry updated successfully', 'success');
      } else {
        const response = await createTimeVoyageOffshore({
          financialYear,
          offshoreFleet: numericValues[0],
          timeCharter: numericValues[1],
          voyageCharter: numericValues[2],
          revenue: numericValues[3],
          average: numericValues[4],
          userID: getCurrentUserId(),
        });
        ...
        triggerNotification && triggerNotification('Time & Voyage Chartered (Offshore) entry added successfully', 'success');
      }
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden border-l-4 border-l-[#0f417a] animate-fade-in">
      <div className="bg-gradient-to-r from-[#0f417a] to-[#1a5ba3] px-6 py-4.5 flex items-center justify-between text-white border-b border-[#0a2d55]/20">
        <div>
          ...
          <p className="text-[10px] text-blue-200 font-semibold tracking-wide mt-0.5">SCI - Vessel Availability/Utilization - Time & Voyage Chartered Ships - Offshore</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {numberField('Total Number of Offshore Vessels in Fleet', offshoreFleet, setOffshoreFleet, 'offshoreFleet', 'e.g. 8')}
          {numberField('Total Number of Days on Time Charter', timeCharter, setTimeCharter, 'timeCharter', 'e.g. 1200')}
          {numberField('Total Number of Days on Voyage Charter', voyageCharter, setVoyageCharter, 'voyageCharter', 'e.g. 900')}
          {numberField('Total Revenue (Rs. in Crs.)', revenue, setRevenue, 'revenue', 'e.g. 180.5')}
          {numberField('Average Earnings per Day (US $)', average, setAverage, 'average', 'e.g. 25000')}
        </div>

...
import { fetchShipDryDockingReport } from '../../api';
...
// Confirmed from the legacy site: column series for Scheduled + line
// series for Completed, one shared axis (both are counts of dry-dockings).
export default function ShipDryDockingReports() {
  ...
  const title = 'Form No. K-6.3.1 - Abstract - SCI - Ship Dry-Docking (Own Ships)';

  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchShipDryDockingReport()
      .then((res) => {
        ...
        const scheduledRow = data.find((r) => r.Metric === 'Total dry docking scheduled for own ships') || {};
        const completedRow = data.find((r) => r.Metric === 'Total dry docking completed for own ships') || {};

        setChartData(yearCols.map((yr) => ({
          year: yr,
          scheduled: Number(scheduledRow[yr] || 0),
          completed: Number(completedRow[yr] || 0),
        })));
  ...
  useLayoutEffect(() => {
    ...
    const scheduledSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Total Dry Docking Scheduled', xAxis, yAxis,
        valueYField: 'scheduled', categoryXField: 'year',
        tooltip: am5.Tooltip.new(root, { labelText: 'Total Dry Docking Scheduled: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    scheduledSeries.columns.template.setAll({ fill: am5.color(0x87ceeb), stroke: am5.color(0x87ceeb), width: am5.percent(50) });
    scheduledSeries.bullets.push((root) => {
    ...
    scheduledSeries.data.setAll(chartData);

    const completedSeries = chart.series.push(
      am5xy.LineSeries.new(root, {
        name: 'Total Dry Docking Completed', xAxis, yAxis,
        valueYField: 'completed', categoryXField: 'year',
        stroke: am5.color(0xff5733), fill: am5.color(0xff5733),
        tooltip: am5.Tooltip.new(root, { labelText: 'Total Dry Docking Completed: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    completedSeries.strokes.template.setAll({ strokeWidth: 2, stroke: am5.color(0xff5733) });
    completedSeries.bullets.push(() => am5.Bullet.new(root, {
      sprite: am5.Circle.new(root, { radius: 4, fill: am5.color(0xff5733), stroke: am5.color(0xffffff), strokeWidth: 2 })
    }));
    completedSeries.bullets.push((root) => {
    ...
    completedSeries.data.setAll(chartData);
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'sci_ship_dry_docking_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
        ...
        fileName: 'sci_ship_dry_docking_report',
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-[#fdfcfc] to-slate-50 dark:from-slate-900 dark:to-slate-800 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        <div className="flex-1 min-w-[300px]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10.5px] font-black text-[#8c4242] dark:text-blue-400 uppercase tracking-widest">SCI - Ship Dry-Docking Report</span>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="sci_ship_dry_docking_chart" color="#4b2424" />
      ...
      {error ? (
        ...
        <div className="ag-theme-quartz sci-drydocking-report-grid mt-4" style={{ width: '100%' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .sci-drydocking-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .sci-drydocking-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .sci-drydocking-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .sci-drydocking-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .sci-drydocking-report-grid .ag-header-cell-label { justify-content: center !important; }
        .sci-drydocking-report-grid .ag-row-even { background: #ffffff !important; }
        .sci-drydocking-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .sci-drydocking-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .sci-drydocking-report-grid .ag-header { background: #0f172a !important; }
        .dark .sci-drydocking-report-grid .ag-row-even { background: #090d16 !important; }
        .dark .sci-drydocking-report-grid .ag-row-odd { background: #0f172a !important; }
        .dark .sci-drydocking-report-grid .ag-cell { border-right: 1px solid #1e293b !important; color: #e2e8f0 !important; }
      `}} />
      </div>
    </div>
  );

...
import { fetchVesselAvailabilityOwnShipsReport } from '../../api';
...
// Confirmed from the legacy site: line series for Own Operated Ships +
// column series for Ship Utilization %, one shared axis.
export default function VesselAvailabilityOwnShipsReports() {
  ...
  const title = 'Form No. K-6.1.1 - Abstract - SCI - Vessel Availability/Utilization(%) - Own Operated Ships';

  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchVesselAvailabilityOwnShipsReport()
      .then((res) => {
        ...
        const operatedRow = data.find((r) => r.Metric === 'No. of Own Operated Ships') || {};
        const utilizationRow = data.find((r) => r.Metric === 'Ship Utilization %') || {};

        setChartData(yearCols.map((yr) => ({
          year: yr,
          operated: Number(operatedRow[yr] || 0),
          utilization: Number(utilizationRow[yr] || 0),
        })));
  ...
  useLayoutEffect(() => {
    ...
    const utilizationSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Ship Utilization %', xAxis, yAxis,
        valueYField: 'utilization', categoryXField: 'year',
        tooltip: am5.Tooltip.new(root, { labelText: 'Ship Utilization %: [bold]{valueY}%[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    utilizationSeries.columns.template.setAll({ fill: am5.color(0x5f9ea0), stroke: am5.color(0x5f9ea0), width: am5.percent(50) });
    utilizationSeries.bullets.push((root) => {
    ...
    utilizationSeries.data.setAll(chartData);

    const operatedSeries = chart.series.push(
      am5xy.LineSeries.new(root, {
        name: 'No. of Own Operated Ships', xAxis, yAxis,
        valueYField: 'operated', categoryXField: 'year',
        stroke: am5.color(0xff5733), fill: am5.color(0xff5733),
        tooltip: am5.Tooltip.new(root, { labelText: 'No. of Own Operated Ships: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    operatedSeries.strokes.template.setAll({ strokeWidth: 2, stroke: am5.color(0xff5733) });
    operatedSeries.bullets.push(() => am5.Bullet.new(root, {
      sprite: am5.Circle.new(root, { radius: 4, fill: am5.color(0xff5733), stroke: am5.color(0xffffff), strokeWidth: 2 })
    }));
    operatedSeries.bullets.push((root) => {
      const label = am5.Label.new(root, { fill: am5.color(0xff5733), centerX: am5.p50, centerY: am5.p0, dy: -26, fontSize: 10, fontWeight: '700', populateText: true, text: '{valueY}' });
      return am5.Bullet.new(root, { sprite: label });
    });
    operatedSeries.data.setAll(chartData);
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'sci_vessel_availability_own_ships_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
        ...
        fileName: 'sci_vessel_availability_own_ships_report',
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        <div className="flex-1 min-w-[300px]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10.5px] font-black text-[#8c4242] dark:text-blue-400 uppercase tracking-widest">SCI - Vessel Availability Report (Own Ships)</span>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="sci_vessel_availability_own_ships_chart" color="#4b2424" />
      ...
      {error ? (
        ...
        <div className="ag-theme-quartz sci-vessel-own-report-grid mt-4" style={{ width: '100%' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .sci-vessel-own-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .sci-vessel-own-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .sci-vessel-own-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .sci-vessel-own-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .sci-vessel-own-report-grid .ag-header-cell-label { justify-content: center !important; }
        .sci-vessel-own-report-grid .ag-row-even { background: #ffffff !important; }
        .sci-vessel-own-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .sci-vessel-own-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .sci-vessel-own-report-grid .ag-header { background: #0f172a !important; }
        .dark .sci-vessel-own-report-grid .ag-row-even { background: #090d16 !important; }
        .dark .sci-vessel-own-report-grid .ag-row-odd { background: #0f172a !important; }
        .dark .sci-vessel-own-report-grid .ag-cell { border-right: 1px solid #1e293b !important; color: #e2e8f0 !important; }
      `}} />
      </div>
    </div>
  );

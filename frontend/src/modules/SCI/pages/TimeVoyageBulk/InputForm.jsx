...
import { createTimeVoyageBulk, updateTimeVoyageBulk } from '../../api';
...
export default function TimeVoyageBulkInputForm({
  editData = null,
  onBack,
  onSuccess,
  triggerNotification,
  ...
  const [bulkCarriers, setBulkCarriers] = useState(editData?.total_bulk_carriers_fleet ?? '');
  const [timeCharter, setTimeCharter] = useState(editData?.total_bulk_carriers_time_charter ?? '');
  const [voyageCharter, setVoyageCharter] = useState(editData?.total_bulk_carriers_voyage_charter ?? '');
  const [revenue, setRevenue] = useState(editData?.total_revenue_bulk_carriers ?? '');
  const [earnings, setEarnings] = useState(editData?.average_earnings_bulk_carriers ?? '');
  ...
  const handleSubmit = async (e) => {
    ...
    const numericValues = [bulkCarriers, timeCharter, voyageCharter, revenue, earnings].map(Number);
    ...
    try {
      if (isEdit) {
        await updateTimeVoyageBulk({
          updatefinancialYear: financialYear,
          updatebulkCarriers: numericValues[0],
          ...
          ScitimeVoyageBuiklIdOrg: editData.sci_time_voyage_bulk_id,
          userID: getCurrentUserId(),
        });
        triggerNotification && triggerNotification('Time & Voyage Chartered (Bulk) entry updated successfully', 'success');
      } else {
        const response = await createTimeVoyageBulk({
          financialYear,
          bulkCarriers: numericValues[0],
          timeCharter: numericValues[1],
          voyageCharter: numericValues[2],
          revenue: numericValues[3],
          earnings: numericValues[4],
          userID: getCurrentUserId(),
        });
        ...
        triggerNotification && triggerNotification('Time & Voyage Chartered (Bulk) entry added successfully', 'success');
      }
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden border-l-4 border-l-[#0f417a] animate-fade-in">
      <div className="bg-gradient-to-r from-[#0f417a] to-[#1a5ba3] px-6 py-4.5 flex items-center justify-between text-white border-b border-[#0a2d55]/20">
        <div>
          ...
          <p className="text-[10px] text-blue-200 font-semibold tracking-wide mt-0.5">SCI - Vessel Availability/Utilization - Time & Voyage Chartered Ships - Bulk</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {numberField('Total Number of Bulk Carriers in Fleet', bulkCarriers, setBulkCarriers, 'bulkCarriers', 'e.g. 15')}
          {numberField('Total Number of Days on Time Charter', timeCharter, setTimeCharter, 'timeCharter', 'e.g. 2100')}
          {numberField('Total Number of Days on Voyage Charter', voyageCharter, setVoyageCharter, 'voyageCharter', 'e.g. 1800')}
          {numberField('Total Revenue (Rs. in Crs.)', revenue, setRevenue, 'revenue', 'e.g. 245.5')}
          {numberField('Average Earnings per Day (US $)', earnings, setEarnings, 'earnings', 'e.g. 18500')}
        </div>

...
import { fetchRepairAndMaintenanceReport } from '../../api';
...
// Confirmed from the legacy site: genuinely a dual-axis chart -- Cost and
// Revenue (both Rs. Lakhs) as clustered columns on the left axis,
// Percentage as a line on the right axis (opposite: true). Unlike IMU's
// Final Year Pass Percentage, which was confirmed single-axis, this one
// really does split the scales. Auto-scaling headroom used here instead
// of the legacy site's hardcoded max: 700000 / max: 12.
export default function RepairAndMaintenanceReports() {
  ...
  const title = 'Form No. K-6.3.2 - Abstract - SCI - Repair & Maintenance Costs as % of Operational Revenue (Own Ships)';

  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchRepairAndMaintenanceReport()
      .then((res) => {
        ...
        const costRow = data.find((r) => r.Metric === 'Total repair and maintance cost(in Rs Lakhs)') || {};
        const revenueRow = data.find((r) => r.Metric === 'Total operational revenue(in Rs Lakhs)') || {};
        const pctRow = data.find((r) => r.Metric === 'Repair & Maintance Costs(%)') || {};

        setChartData(yearCols.map((yr) => ({
          year: yr,
          cost: Number(costRow[yr] || 0),
          revenue: Number(revenueRow[yr] || 0),
  ...
  useLayoutEffect(() => {
    ...
    const costSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Total Repair & Maintenance Cost (Rs. Lakhs)', xAxis, yAxis: yAxisLeft,
        valueYField: 'cost', categoryXField: 'year',
        clustered: true,
        tooltip: am5.Tooltip.new(root, { labelText: 'Total Repair & Maintenance Cost (Rs. Lakhs): [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    costSeries.columns.template.setAll({ fill: am5.color(0x50c878), stroke: am5.color(0x50c878), width: am5.percent(70) });
    costSeries.data.setAll(chartData);

    const revenueSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Total Operational Revenue (Rs. Lakhs)', xAxis, yAxis: yAxisLeft,
        valueYField: 'revenue', categoryXField: 'year',
        clustered: true,
        tooltip: am5.Tooltip.new(root, { labelText: 'Total Operational Revenue (Rs. Lakhs): [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    revenueSeries.columns.template.setAll({ fill: am5.color(0x87ceeb), stroke: am5.color(0x87ceeb), width: am5.percent(70) });
    revenueSeries.data.setAll(chartData);

    const pctSeries = chart.series.push(
      am5xy.LineSeries.new(root, {
        name: 'Repair & Maintenance Costs (%)', xAxis, yAxis: yAxisRight,
        valueYField: 'percentage', categoryXField: 'year',
        stroke: am5.color(0xd4a017), fill: am5.color(0xd4a017),
        tooltip: am5.Tooltip.new(root, { labelText: 'Repair & Maintenance Costs (%): [bold]{valueY}%[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'sci_repair_and_maintenance_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
        ...
        fileName: 'sci_repair_and_maintenance_report',
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-[#fdfcfc] to-slate-50 dark:from-slate-900 dark:to-slate-800 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        <div className="flex-1 min-w-[300px]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10.5px] font-black text-[#8c4242] dark:text-blue-400 uppercase tracking-widest">SCI - Repair & Maintenance Report</span>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="sci_repair_and_maintenance_chart" color="#4b2424" />
      ...
      {error ? (
        ...
        <div className="ag-theme-quartz sci-repair-maintenance-report-grid mt-4" style={{ width: '100%' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .sci-repair-maintenance-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .sci-repair-maintenance-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .sci-repair-maintenance-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .sci-repair-maintenance-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .sci-repair-maintenance-report-grid .ag-header-cell-label { justify-content: center !important; }
        .sci-repair-maintenance-report-grid .ag-row-even { background: #ffffff !important; }
        .sci-repair-maintenance-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .sci-repair-maintenance-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .sci-repair-maintenance-report-grid .ag-header { background: #0f172a !important; }
        .dark .sci-repair-maintenance-report-grid .ag-row-even { background: #090d16 !important; }
        .dark .sci-repair-maintenance-report-grid .ag-row-odd { background: #0f172a !important; }
        .dark .sci-repair-maintenance-report-grid .ag-cell { border-right: 1px solid #1e293b !important; color: #e2e8f0 !important; }
      `}} />
      </div>
    </div>
  );

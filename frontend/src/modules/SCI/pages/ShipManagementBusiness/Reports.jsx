...
import { fetchShipManagementBusinessReport } from '../../api';
...
// Confirmed from the legacy site: only 2 of the 4 stored metrics are
// charted (Number of Ships Managed as a column, Cost to Revenue Ratio as
// a line on a dual axis) -- Total Management Cost and Revenue are shown
// in the table only, matching the legacy source exactly. Also confirmed
// the backend's year-column query had a real bug (pulling years from the
// Green Recycling table instead of this table); that bug was fixed as
// part of building this report.
export default function ShipManagementBusinessReports() {
  ...
  const title = 'Form No. K-6.6.1 - Abstract - SCI - Ship Management Business - Cost to Revenue Ratio from Management of Ships';

  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchShipManagementBusinessReport()
      .then((res) => {
        ...
        const shipsRow = data.find((r) => r.Metric === 'Number of ships managed by SCI') || {};
        const ratioRow = data.find((r) => r.Metric === 'Cost to revenue ratio (%)') || {};

        setChartData(yearCols.map((yr) => ({
          year: yr,
          ships: Number(shipsRow[yr] || 0),
          ratio: Number(ratioRow[yr] || 0),
        })));
  ...
  useLayoutEffect(() => {
    ...
    const shipsSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Number of Ships Managed by SCI', xAxis, yAxis: yAxisLeft,
        valueYField: 'ships', categoryXField: 'year',
        tooltip: am5.Tooltip.new(root, { labelText: 'Number of Ships Managed by SCI: [bold]{valueY}[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    shipsSeries.columns.template.setAll({ fill: am5.color(0x50c878), stroke: am5.color(0x50c878), width: am5.percent(60) });
    shipsSeries.bullets.push((root) => {
    ...
    shipsSeries.data.setAll(chartData);

    const ratioSeries = chart.series.push(
      am5xy.LineSeries.new(root, {
        name: 'Cost to Revenue Ratio (%)', xAxis, yAxis: yAxisRight,
        valueYField: 'ratio', categoryXField: 'year',
        stroke: am5.color(0xd4a017), fill: am5.color(0xd4a017),
        tooltip: am5.Tooltip.new(root, { labelText: 'Cost to Revenue Ratio (%): [bold]{valueY}%[/]', pointerOrientation: 'horizontal', centerY: am5.percent(100) }),
      })
    );
    ratioSeries.strokes.template.setAll({ strokeWidth: 2, stroke: am5.color(0xd4a017) });
    ratioSeries.bullets.push(() => am5.Bullet.new(root, {
      sprite: am5.Circle.new(root, { radius: 4, fill: am5.color(0xd4a017), stroke: am5.color(0xffffff), strokeWidth: 2 })
    }));
    ratioSeries.bullets.push((root) => {
    ...
    ratioSeries.data.setAll(chartData);
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'sci_ship_management_business_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
        ...
        fileName: 'sci_ship_management_business_report',
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        <div className="flex-1 min-w-[300px]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10.5px] font-black text-[#8c4242] dark:text-blue-400 uppercase tracking-widest">SCI - Ship Management Business Report</span>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="sci_ship_management_business_chart" color="#4b2424" />
      ...
      {error ? (
        ...
        <div className="ag-theme-quartz sci-ship-management-report-grid mt-4" style={{ width: '100%' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .sci-ship-management-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .sci-ship-management-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .sci-ship-management-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .sci-ship-management-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .sci-ship-management-report-grid .ag-header-cell-label { justify-content: center !important; }
        .sci-ship-management-report-grid .ag-row-even { background: #ffffff !important; }
        .sci-ship-management-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .sci-ship-management-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .sci-ship-management-report-grid .ag-header { background: #0f172a !important; }
        .dark .sci-ship-management-report-grid .ag-row-even { background: #090d16 !important; }
        .dark .sci-ship-management-report-grid .ag-row-odd { background: #0f172a !important; }
        .dark .sci-ship-management-report-grid .ag-cell { border-right: 1px solid #1e293b !important; color: #e2e8f0 !important; }
      `}} />
      </div>
    </div>
  );

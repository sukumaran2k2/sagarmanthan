...
import { createTimeVoyageTanker, updateTimeVoyageTanker } from '../../api';
...
export default function TimeVoyageTankerInputForm({
  editData = null,
  onBack,
  onSuccess,
  triggerNotification,
  ...
  const [totalTankersFleet, setTotalTankersFleet] = useState(editData?.total_no_of_tankers_in_fleet ?? '');
  const [daysTimeCharter, setDaysTimeCharter] = useState(editData?.total_no_days_on_time_charter ?? '');
  const [daysVoyageCharter, setDaysVoyageCharter] = useState(editData?.total_no_days_on_voyage_charter ?? '');
  const [totalRevenue, setTotalRevenue] = useState(editData?.total_revenue_tankers ?? '');
  const [averageEarnings, setAverageEarnings] = useState(editData?.average_earnings_tankers ?? '');
  ...
  const FIELDS = [
    ...
    ['totalTankersFleet', totalTankersFleet],
    ['daysTimeCharter', daysTimeCharter],
    ['daysVoyageCharter', daysVoyageCharter],
  ...
  const handleSubmit = async (e) => {
    ...
    const numericValues = [totalTankersFleet, daysTimeCharter, daysVoyageCharter, totalRevenue, averageEarnings].map(Number);
    ...
    try {
      if (isEdit) {
        await updateTimeVoyageTanker({
          updatefinancialYear: financialYear,
          updatetotalTankersFleet: numericValues[0],
          updatedaysTimeCharter: numericValues[1],
          updatedaysVoyageCharter: numericValues[2],
          updateRevenue: numericValues[3],
          updateTanker: numericValues[4],
          ScitimeVoyageTankerlIdOrg: editData.sci_time_voyage_tanker_id,
          userID: getCurrentUserId(),
        });
        triggerNotification && triggerNotification('Time & Voyage Chartered (Tanker) entry updated successfully', 'success');
      } else {
        const response = await createTimeVoyageTanker({
          financialYear,
          totalTankersFleet: numericValues[0],
          daysTimeCharter: numericValues[1],
          daysVoyageCharter: numericValues[2],
          totalRevenue: numericValues[3],
          averageEarnings: numericValues[4],
          userID: getCurrentUserId(),
        });
        ...
        triggerNotification && triggerNotification('Time & Voyage Chartered (Tanker) entry added successfully', 'success');
      }
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden border-l-4 border-l-[#0f417a] animate-fade-in">
      <div className="bg-gradient-to-r from-[#0f417a] to-[#1a5ba3] px-6 py-4.5 flex items-center justify-between text-white border-b border-[#0a2d55]/20">
        <div>
          ...
          <p className="text-[10px] text-blue-200 font-semibold tracking-wide mt-0.5">SCI - Vessel Availability/Utilization - Time & Voyage Chartered Ships - Tanker</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {numberField('Total Number of Tankers in Fleet', totalTankersFleet, setTotalTankersFleet, 'totalTankersFleet', 'e.g. 12')}
          {numberField('Total Number of Days on Time Charter', daysTimeCharter, setDaysTimeCharter, 'daysTimeCharter', 'e.g. 1800')}
          {numberField('Total Number of Days on Voyage Charter', daysVoyageCharter, setDaysVoyageCharter, 'daysVoyageCharter', 'e.g. 1500')}
          {numberField('Total Revenue (Rs. in Crs.)', totalRevenue, setTotalRevenue, 'totalRevenue', 'e.g. 320.5')}
          {numberField('Average Earnings per Day (US $)', averageEarnings, setAverageEarnings, 'averageEarnings', 'e.g. 22000')}
        </div>

...
export default function NAISIntegrationReports() {
  ...
  useLayoutEffect(() => {
    ...
    const chart = root.container.children.push(
      am5xy.XYChart.new(root, { panX: false, panY: false, wheelX: 'none', wheelY: 'none', layout: root.verticalLayout, paddingTop: 10, paddingBottom: 0, paddingRight: 45 })
    );
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'nais_integration_report.csv' });
      ...
      exportReportToPdf({
        ...
        fileName: 'nais_integration_report',
      });
    }
  };
    ...
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-[#fdfcfc] to-slate-50 dark:from-slate-900 dark:to-slate-800 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="nais_integration_chart" color="#4b2424" />

...
import { fetchNaisUptimeReport } from '../../api';
...
export default function NAISUptimeReports() {
  ...
  const title = 'Form No. DGLL K-3.3 - Abstract - NAIS Uptime';

  const fetchData = () => {
    setLoading(true);
    setError(null);
    fetchNaisUptimeReport()
      .then((res) => {
      ...
      .catch((err) => {
        console.error('Error loading NAIS Uptime report:', err);
  ...
  useLayoutEffect(() => {
    ...
    const chart = root.container.children.push(
      am5xy.XYChart.new(root, { panX: false, panY: false, wheelX: 'none', wheelY: 'none', layout: root.verticalLayout, paddingTop: 10, paddingBottom: 0, paddingRight: 20 })
    );
    ...
    // Uptime is a percentage -- cap the axis at 100 with a little headroom for labels.
    const yAxis = chart.yAxes.push(am5xy.ValueAxis.new(root, { renderer: yRenderer, min: 0, max: 100, extraMax: 0.08 }));

    const barSeries = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: 'Availability / Uptime of NAIS (%)', xAxis, yAxis,
        valueYField: 'value', categoryXField: 'year',
        tooltip: am5.Tooltip.new(root, { labelText: 'Availability / Uptime of NAIS: [bold]{valueY}%[/]' }),
      })
    );
  ...
  const handleExport = (type) => {
    if (type === 'Excel' && gridRef.current?.api) gridRef.current.api.exportDataAsCsv({ fileName: 'nais_uptime_report.csv' });
    else if (type === 'PDF') {
      exportReportToPdf({
        title,
  ...
  return (
    <div className="rounded-2xl shadow-lg">
      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4 px-6 py-6 bg-gradient-to-r from-[#fdfcfc] to-slate-50 dark:from-slate-900 dark:to-slate-800 border-b border-slate-200 dark:border-slate-800 rounded-t-2xl">
        <div className="flex-1 min-w-[300px]">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10.5px] font-black text-[#8c4242] dark:text-blue-400 uppercase tracking-widest">DGLL - NAIS Uptime Report</span>
          </div>
        ...
        <div className="flex items-center justify-end gap-2.5 flex-wrap">
          ...
          <ChartExportMenu chartRoot={chartRoot} fileName="nais_uptime_chart" color="#4b2424" />
      ...
      {error ? (
        ...
        <div className="ag-theme-quartz nais-uptime-report-grid mt-4" style={{ width: '100%' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .nais-uptime-report-grid.ag-theme-quartz { --ag-font-family: 'Inter', system-ui, sans-serif; --ag-row-height: 48px; --ag-active-color: #4b2424; }
        .nais-uptime-report-grid .ag-root-wrapper { border: none !important; border-radius: 0 !important; }
        .nais-uptime-report-grid .ag-header { background: #4b2424 !important; border-bottom: 2px solid #3a1a1a !important; }
        .nais-uptime-report-grid .ag-header-cell { color: #ffffff !important; font-weight: 600 !important; font-size: 11px !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; border-right: 1px solid rgba(255,255,255,0.15) !important; }
        .nais-uptime-report-grid .ag-header-cell-label { justify-content: center !important; }
        .nais-uptime-report-grid .ag-row-even { background: #ffffff !important; }
        .nais-uptime-report-grid .ag-row-odd { background: #f8faf6 !important; }
        .nais-uptime-report-grid .ag-cell { border-right: 1px solid #D3D6D9 !important; }
        .dark .nais-uptime-report-grid .ag-header { background: #0f172a !important; }
        .dark .nais-uptime-report-grid .ag-row-even { background: #090d16 !important; }
        .dark .nais-uptime-report-grid .ag-row-odd { background: #0f172a !important; }
        .dark .nais-uptime-report-grid .ag-cell { border-right: 1px solid #1e293b !important; color: #e2e8f0 !important; }
      `}} />
      </div>
    </div>
  );

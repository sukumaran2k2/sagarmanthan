...
export async function getLiquidBulkSingleLeg(req, res) {
    ...
    const query = `
           DECLARE @cols NVARCHAR(MAX), @sql NVARCHAR(MAX);
            ...
            -- Step 1: Generate dynamic column names
            SELECT @cols = STRING_AGG(QUOTENAME(pivot_col), ',')
            FROM (
                SELECT DISTINCT
                ...
                WHERE 
                    cd.commodity_group_id = 1
                    AND cd.commodity_subcategory_id IN (1,2,3,4,5,6,7)
                    AND cd.category_id IN (1,2,3)

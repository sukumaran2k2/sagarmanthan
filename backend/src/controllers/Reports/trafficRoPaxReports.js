...
async function getRoPaxTraffic_k_1_9_1_Report(req, res) {
    ...
    try {
        ...
        LastFiscalYearTarget AS (
            SELECT 
                organisation_id, 
                NULLIF(SUM(ro_pax_target), 0) AS lastFiscalYearTarget
            FROM tbl_traffic_fiscal_target
            WHERE financial_year = @lastFiscalYear
            GROUP BY organisation_id
        ),
        CurrentFiscalYearTarget AS (
            SELECT 
                organisation_id, 
                NULLIF(SUM(ro_pax_target), 0) AS currentFiscalYearTarget
            FROM tbl_traffic_fiscal_target
        ...
        console.error("Error fetching RoPax traffic report:", error);
        return res.status(500).json({ error: "Internal Server Error" });
    }
}

async function getRoPaxTraffic_k_1_9_2_Report(req, res) {
    const conn = await pool;
    try {
        const currentDate = new Date();
        const currentYear = currentDate.getFullYear();
        ...
        WITH MonthlyData AS (
            SELECT 
                ...
                fiscal_year,
                month,
                SUM(ro_pax_traffic) AS total_traffic
            FROM tbl_commodity_traffic
    ...
    `;
        ...
        console.error("Error fetching RoPax Pax traffic report:", error);
        return res.status(500).json({ error: "Internal Server Error" });
    }
}

async function getRoPaxTraffic_k_1_9_3_Report(req, res) {
    const conn = await pool;
    try {
        const currentDate = new Date();
        const currentYear = currentDate.getFullYear();
        ...
        const query = `
            WITH YearlyData AS (
                SELECT 
                    ...
                    o.organisation_name AS majorPort,
                    c.fiscal_year,
                    SUM(c.ro_pax_traffic) AS total_traffic
                FROM tbl_commodity_traffic c
        ...
        console.error("Error fetching year-wise RoPax traffic report:", error);
        return res.status(500).json({ error: "Internal Server Error" });
    }
}

export default { getRoPaxTraffic_k_1_9_1_Report, getRoPaxTraffic_k_1_9_2_Report, getRoPaxTraffic_k_1_9_3_Report
 };

...
async function getContainerTrtPortPerformance_k_1_21_1_Report(req, res) {
            ...
            SELECT 
                ...
                CASE 
                    ...
                    THEN NULLIF(SUM(CASE 
                        ...
                        THEN average_container_trt * container_vessel_handled 
                        ELSE 0 
                    END), 0) 
                    / 
                    NULLIF(SUM(CASE 
...
async function getContainerTrtPortPerformance_k_1_21_2_Report(req, res) {
    const conn = await pool;
    try {
        ...
        const query = `
            WITH MonthlyData AS (
                SELECT 
                    ...
                    fiscal_year,
                    month,
                    average_container_trt,
                    container_vessel_handled,
                    average_container_trt * container_vessel_handled AS weighted_trt
                FROM tbl_kpi_time_performance
            ),
            PortMonthlyData AS (
                SELECT 
            ...
            PivotedPortData AS (
                SELECT 
                    ...
                    majorPort,
                    fiscal_year,
                    SUM(CASE WHEN month = 4 THEN average_container_trt END) AS April,
                    SUM(CASE WHEN month = 5 THEN average_container_trt END) AS May,
                    SUM(CASE WHEN month = 6 THEN average_container_trt END) AS June,
                    SUM(CASE WHEN month = 7 THEN average_container_trt END) AS July,
                    SUM(CASE WHEN month = 8 THEN average_container_trt END) AS August,
                    SUM(CASE WHEN month = 9 THEN average_container_trt END) AS September,
                    SUM(CASE WHEN month = 10 THEN average_container_trt END) AS October,
                    SUM(CASE WHEN month = 11 THEN average_container_trt END) AS November,
                    SUM(CASE WHEN month = 12 THEN average_container_trt END) AS December,
                    SUM(CASE WHEN month = 1 THEN average_container_trt END) AS January,
                    SUM(CASE WHEN month = 2 THEN average_container_trt END) AS February,
                    SUM(CASE WHEN month = 3 THEN average_container_trt END) AS March
            ...
            WeightedTotalData AS (
                SELECT 
                    ...
                    NULLIF(SUM(CASE WHEN month = 4 THEN container_vessel_handled END), 0) AS April,
                    NULLIF(SUM(CASE WHEN month = 5 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 5 THEN container_vessel_handled END), 0) AS May,
                    NULLIF(SUM(CASE WHEN month = 6 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 6 THEN container_vessel_handled END), 0) AS June,
                    NULLIF(SUM(CASE WHEN month = 7 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 7 THEN container_vessel_handled END), 0) AS July,
                    NULLIF(SUM(CASE WHEN month = 8 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 8 THEN container_vessel_handled END), 0) AS August,
                    NULLIF(SUM(CASE WHEN month = 9 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 9 THEN container_vessel_handled END), 0) AS September,
                    NULLIF(SUM(CASE WHEN month = 10 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 10 THEN container_vessel_handled END), 0) AS October,
                    NULLIF(SUM(CASE WHEN month = 11 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 11 THEN container_vessel_handled END), 0) AS November,
                    NULLIF(SUM(CASE WHEN month = 12 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 12 THEN container_vessel_handled END), 0) AS December,
                    NULLIF(SUM(CASE WHEN month = 1 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 1 THEN container_vessel_handled END), 0) AS January,
                    NULLIF(SUM(CASE WHEN month = 2 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 2 THEN container_vessel_handled END), 0) AS February,
                    NULLIF(SUM(CASE WHEN month = 3 THEN weighted_trt END), 0) / 
                    NULLIF(SUM(CASE WHEN month = 3 THEN container_vessel_handled END), 0) AS March
                FROM MonthlyData
...
async function getContainerTrtPortPerformance_k_1_21_3_Report(req, res) {
    const conn = await pool;
    try {
        const currentDate = new Date();
        const currentYear = currentDate.getFullYear();
        ...
        const fiscalYearColumns = fiscalYears.map(year => {
            return `
                NULLIF(SUM(CASE WHEN yd.fiscal_year = '${year}' 
                    THEN yd.average_container_trt * yd.container_vessel_handled ELSE 0 END), 0)
                /
                NULLIF(SUM(CASE WHEN yd.fiscal_year = '${year}' 
                    THEN yd.container_vessel_handled ELSE 0 END), 0)
                AS "FY${year}"
            `;
        }).join(',\n');
        ...
        const totalRowColumns = fiscalYears.map(year => {
            return `
                NULLIF(SUM(CASE WHEN fiscal_year = '${year}' 
                    THEN average_container_trt * container_vessel_handled ELSE 0 END), 0)
                /
                NULLIF(SUM(CASE WHEN fiscal_year = '${year}' 
                    THEN container_vessel_handled ELSE 0 END), 0)
                AS "FY${year}"
            `;
        }).join(',\n');
...
export default {
    getContainerTrtPortPerformance_k_1_21_1_Report, getContainerTrtPortPerformance_k_1_21_2_Report, getContainerTrtPortPerformance_k_1_21_3_Report
};

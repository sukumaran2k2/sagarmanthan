...
async function getRoRoTraffic_k_1_8_1_Report(req, res) {
    ...
    try {
        ...
        LastFiscalYearTarget AS (
            SELECT 
                organisation_id, 
                NULLIF(SUM(ro_ro_target), 0) AS lastFiscalYearTarget
        ...
        CurrentFiscalYearTarget AS (
            SELECT 
                organisation_id, 
                NULLIF(SUM(ro_ro_target), 0) AS currentFiscalYearTarget
            FROM tbl_traffic_fiscal_target
...
async function getRoRoTraffic_k_1_8_2_Report(req, res) {
    const conn = await pool;
    try {
        const currentDate = new Date();
        const currentYear = currentDate.getFullYear();
        ...
        WITH MonthlyData AS (
            SELECT 
                ...
                fiscal_year,
                month,
                SUM(ro_ro_traffic) AS total_traffic
            FROM tbl_commodity_traffic
...
async function getRoRoTraffic_k_1_8_3_Report(req, res) {
    const conn = await pool;
    try {
        const currentDate = new Date();
        const currentYear = currentDate.getFullYear();
        ...
        const query = `
            WITH YearlyData AS (
                SELECT 
                    ...
                    o.organisation_name AS majorPort,
                    c.fiscal_year,
                    SUM(c.ro_ro_traffic) AS total_traffic
        ...
        console.error("Error fetching year-wise RoRo traffic report:", error);
        return res.status(500).json({ error: "Internal Server Error" });
    }
}

export default { getRoRoTraffic_k_1_8_1_Report, getRoRoTraffic_k_1_8_2_Report, getRoRoTraffic_k_1_8_3_Report
 };
